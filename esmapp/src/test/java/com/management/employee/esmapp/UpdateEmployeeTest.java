...
public class UpdateEmployeeTest {
	...
	Logger logger = LoggerFactory.getLogger(UpdateEmployeeTest.class);
	...
	public void updateEmployeeSuccessTest() throws Exception {
		EmployeeData empData = new EmployeeData("e0001", "hpotter", "Harry Potter", 4500.0f);
		
		MockMvc mockMvc = MockMvcBuilders.webAppContextSetup(webApplicationContext).build();
		mockMvc.perform(patch("/users/{id}", empData.getId())
				 .contentType(MediaType.APPLICATION_JSON)
				.content(convertToJson(empData)))
		.andExpect(status().isOk())
		.andExpect(jsonPath("$.responseCode").value(HttpStatus.OK.value()))
		.andExpect(jsonPath("$.message").value("Employee data updated successfully"));
	}
	
	@Test
	public void employeeLoginExistTest() throws Exception {
		EmployeeData empData = new EmployeeData("e0001", "gwesley", "Ginny Weasley", 1555.0f);
		...
		.andExpect(jsonPath("$.message").value("Employee login already exists. Please choose another login."));
	}
